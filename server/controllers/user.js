...
const registerGuest = asyncHandler(async (req, res) => {
  ...
  }/api/user/finalregister?${queryParams.toString()}`;
  const html = `Yêu cầu click vào link ở dưới để hoàn tất quá trình đăng ký. Thời gian link có hiệu lực là 10 phút kể từ khi bạn nhận được. <a href="${link}">Click here</a>`;
  const data = {
    email,
    html,
...
const finalRegister = asyncHandler(async (req, res) => {
  const { email, password, name, phone, token } = req.query;
...
const getUserCart = asyncHandler(async (req, res) => {
  ...
  const user = await User.findById(_id).select(
    "-refreshToken -password -__v -createdAt -updatedAt"
  );
...
const getUser = asyncHandler(async (req, res) => {
  ...
  let queryCommand = User.find(formatedQueries).select(
    "-refreshToken -password -__v -passwordResetToken -passwordResetTokenTimeout"
  );
...
const removeProductFromCart = asyncHandler(async (req, res) => {
  ...
  const alreadyProduct = cart?.products.find(
    (product) => product.product.toString() === pid
  );
  if (!alreadyProduct) {
    return res.status(404).json({
